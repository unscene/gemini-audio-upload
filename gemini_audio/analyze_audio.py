...
def analyze_audio_content(audio_path, prompt, json_path=None, json_context=None, model_name="gemini-3-pro-preview", api_key=None, system_instruction=None):
    if not api_key:
...
def main():
    ...
    try:
        result = analyze_audio_content(audio_path=args.audio, prompt=args.prompt, json_path=args.json, json_context=None, model_name=args.model, api_key=api_key)
        print("\nResponse:")
        print(result)
